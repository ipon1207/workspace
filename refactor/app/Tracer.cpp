...
const int bias = 0;
/*
 * (左右のモーターの個体差を埋めるもの、右モーターが左よりも強い場合は正の値、逆の場合は負の値を設定する）
...
void Tracer::run() {
    //  障害物回避モードの場合
    if (avoid_mode == 1) {
        const uint32_t turn_time = 1000000; // 回避動作の継続時間(マイクロ秒)
...
float Tracer::calc_prop_value() {
  ...
  const int tracemode_lr = mode_lr;
  const float Kp = 0.60;
  const int target = 50;
...
float Tracer::calc_prop_value() {
    ...
    if (target_D != -1) {
        diff_D = colorSensor.getReflection() - target_D;
    }
